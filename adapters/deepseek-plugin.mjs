 ...
 */
export function installDeepSeekObserver(ctx, { boundary, identity, onError = () => {} }) {
  if (typeof ctx?.on !== 'function' || typeof identity !== 'function') throw new TypeError('Context and identity resolver required');
  ...
  const offPre = ctx.on('tools/pre-execute', async (exec, next) => {
    try {
      if (!exec.signal.aborted) await boundary.before(fromDeepSeekCall(exec, identity(exec)));
    } catch { warn(); }
