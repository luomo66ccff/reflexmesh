...
export function validateLedgerSnapshot(value) {
  ...
  check([1, 2, 3].includes(value.ledgerSchemaVersion) && hash(value.logicalDigest));
  check(['wal', 'delete'].includes(value.journalMode));
  const tables = COMPACTION_TABLES.slice(0, value.ledgerSchemaVersion === 1 ? 5 : value.ledgerSchemaVersion === 2 ? 6 : 7);
  fields(value.rowCounts, COMPACTION_TABLES);
  check(COMPACTION_TABLES.every(table => tables.includes(table) ? integer(value.rowCounts[table]) : value.rowCounts[table] === null));
...
export function validateCompactionPlan(value) {
  ...
  check(value.schemaVersion === 1 && value.kind === 'reflexmesh-ledger-compaction-plan');
...
export function logicalEquivalent(a, b) {
  return a.ledgerSchemaVersion === b.ledgerSchemaVersion && a.logicalDigest === b.logicalDigest
    && COMPACTION_TABLES.every(table => a.rowCounts[table] === b.rowCounts[table]);
}
