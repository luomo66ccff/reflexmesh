...
- [x] Preview a selected installed DeepSeek profile's composed configuration in an isolated temporary home so the host's boot-free dump does not rewrite the source profile's generated root file. This is not live default-profile loading; see [profile-preview validation](VALIDATION-DEEPSEEK-PROFILE-PREVIEW-T001.md).
- [x] Optionally export the successfully previewed DeepSeek overlay to a new-only file outside the real profile home, with exact readback and a hash receipt. No real-profile application or host start is implied; see [export validation](VALIDATION-DEEPSEEK-PROFILE-PREVIEW-T002.md).
- [x] Boot the installed DeepSeek CLI/Agent with the generated Loader row supplied only through a separate `--patch` overlay, checking actual Loader activation and native synthetic tool/outcome evidence. This is an isolated empty-bundle profile, not selected/default-profile acceptance; see [overlay-boot validation](VALIDATION-DEEPSEEK-OVERLAY-BOOT-T001.md).
- [x] Opt-in boot a bounded copy of an explicitly selected installed `web` bundle/patch stack with a temporary Loader overlay, loopback ephemeral server and one-shot readiness/cleanup receipt. This executes trusted selected plugins; checked source config remained unchanged, but no real-home, model or tool-flow acceptance is implied. See [selected-web validation](VALIDATION-DEEPSEEK-SELECTED-WEB-BOOT-T001.md).
- [x] Drive one synthetic native read through the actual selected `web` home after private archive readback, redirecting test ledger/session/storage/pet writes to temporary paths and auditing non-dependency home files. This closes one real-profile compatibility gap, not browser UI, paid-model or arbitrary-plugin acceptance; see [real-home validation](VALIDATION-DEEPSEEK-REAL-PROFILE-T001.md).
- [x] Send one fixed synthetic task through the actual selected Web browser UI in a copied, disposable profile, then retain one new-only inspectable shadow ledger. The local installed 0.1.2-rc.1 run passed 21/21 checks; this is neither real-home browser acceptance nor paid-model validation. See [browser validation](VALIDATION-DEEPSEEK-WEB-FIRST-EVIDENCE-T001.md).
...
- [x] Preview bounded historical policy impact under an anchor's exact tenant/source/mode, source pack and deployment binding. Report transitions, changed keys, exclusions and incomplete coverage without a model, tool or ledger write; see the [impact guide](EVIDENCE.md#preview-historical-policy-impact). This remains a local hypothetical, not model-quality or host-outcome evidence.
...
- [x] Generate an explicit Codex STDIO MCP fragment with a read-only first-run doctor; verify generated fields with an installed Codex CLI parser and exercise the production MCP process in an isolated, account-free evidence probe. This is not real Agent or default-profile acceptance; see [guide](CODEX-FIRST-RUN.md) and [local validation](VALIDATION-CODEX-FIRST-RUN-T001.md).
- [x] Make Codex registration reviewable without editing the profile: an opt-in read-only same-name `mcp list --json` check distinguishes absent, matching and conflicting rows, and only an absent row with ready prerequisites receives a copyable `mcp add` command. Actual default-profile installation and loading remain open; see the [first-run guide](CODEX-FIRST-RUN.md) and [local validation](VALIDATION-CODEX-REGISTRATION-PREVIEW-T001.md).
- [x] Extend that account-free production MCP probe across two processes: an identical model-reported call reuses its durable decision, while changed task evidence under the same call identity is rejected. This is not an Agent resume or native-tool retry; see [restart validation](VALIDATION-CODEX-MCP-RESTART-T001.md).
- [x] Add an opt-in logged-in Codex Agent probe for one synthetic assess/native-read/outcome loop, validating typed JSONL and public ledger evidence without changing the user's profile. One isolated Windows run passed; provenance stays model-reported and default-profile acceptance remains open. See [validation](VALIDATION-CODEX-AGENT-T001.md).
...
- [x] Reject out-of-range or fractional in-process decision/action deadlines at construction and direct deadline entry, preventing Node timer overflow from masquerading as an immediate provider failure or unknown action. See [Issue #72](https://github.com/luomo66ccff/reflexmesh/issues/72) and [local validation](VALIDATION-DEADLINE-RANGE-T001.md).
...
`before`/`after` permanent-pending case and a supervised synthetic native-body
nonreturn now have installed-host checks. Natural completion of a genuinely
stuck host tool is not claimed; arbitrary callbacks, event-loop blocking,
default-profile behavior and broader host timeout policy remain open, followed by representative
...
The separate [missing native-result validation](VALIDATION-DEEPSEEK-MISSING-RESULT-T001.md)
