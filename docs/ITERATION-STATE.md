...
layers to a disposable home, then boots the installed selected bundle stack
with an abstain-only CLI overlay and one-shot readiness fixture. It forces
loopback/ephemeral Web binding and no browser launch. The installed 0.1.2-rc.1
`web` stack passed **8/8** startup, Loader, CLI-overlay and natural-cleanup
checks; checked source config and generated root hash/mtime were unchanged,
and the temporary home was removed. It executes selected third-party code;
credentials/settings were not copied, but patch secrets and ambient plugin
access remain possible. It does not prove real-home install, user tool flow or
model inference. See [local validation](VALIDATION-DEEPSEEK-SELECTED-WEB-BOOT-T001.md).

## Previous merged milestone: installed DeepSeek CLI overlay boot

On 2026-09-25, the isolated installed CLI/Agent probe was changed to place
the ReflexMesh Loader row in a separate `--patch` file generated by the same
helper as profile preview, instead of embedding it in the fixture profile.
The installed 0.1.2-rc.1 CLI passed **13/13** checks, including a base profile
without the observer, active Loader entry via CLI overlay, native synthetic
tool/result evidence, abstain binding and natural cleanup. This closes an
overlay-runtime gap, not selected/default-profile, real-model or arbitrary
plugin compatibility. See [local validation](VALIDATION-DEEPSEEK-OVERLAY-BOOT-T001.md).

## Previous merged milestone: new-only DeepSeek overlay export
...
run's pack ID/version, row and full-body digests, event type, question digest
and prediction before exporting the original full pack. Instructions appear
only in the new local file, not the CLI receipt; the ledger is not changed.
The retained synthetic first-run guide now includes a copyable exact-key
export step. In a fresh Chinese/spaced Windows path, source export followed by
policy replay returned `allow -> allow`; the edited fixture candidate still
returned `allow -> escalate`, and the SQLite main-file hash stayed unchanged.
`npm ci --ignore-scripts` and `npm run demo` passed. Local `npm run check`
passed 931/933 tests with zero failures and two symlink-privilege skips;
focused tests after the final wording and binding-guard corrections passed. See
[local validation](VALIDATION-PACK-TEMPLATE-T001.md). This does not authenticate
the ledger, test a real user profile, prove model quality or close the broad
host lifecycle gate. PR-head/main CI remain separate checks.

## Previous merged milestone: policy-only replay from a retained decision
...
verification gates.
[PR #60](https://github.com/luomo66ccff/reflexmesh/pull/60) merged the
reviewed tree `41b54270391b1e9df490614947f5f9f7c0c2a080` unchanged;
all five configured jobs passed on its exact head and on
[main](https://github.com/luomo66ccff/reflexmesh/actions/runs/36097325359).
...
`after` storage continuation never resolves. Both used the explicit fenced
drain deadline and passed 8/8 fixed assertions with a single native tool
result and natural Agent exit. The detached `before` left no decision or false
missing-result count; the detached `after` retained its prior shadow decision
with a missing observer outcome visible in attention. Existing fence and
teardown host probes stayed 10/10 and 9/9. `npm run check` passed 907/909 tests
with zero failures and two local symlink skips; `npm run demo` passed. See the
...
On 2026-09-25, an opt-in probe exercised a logged-in installed Codex Agent
against one fresh synthetic fixture: advisory `reflexmesh_assess`, one native
read, then `reflexmesh_observe_outcome`. The seventh isolated attempt passed
strict typed JSONL and public evidence CLI/direct SQLite readback. It recorded
one ready task and one succeeded outcome with **model-reported** provenance,
zero labels and no ReflexMesh provider egress. The earlier six attempts were
retained as failures while the Windows shell-command verifier was corrected;
they were not converted into passes. `npm run check` passed 903/905 tests with
zero failures and two local symlink skips; `npm run demo` passed. See the
[Agent validation](VALIDATION-CODEX-AGENT-T001.md) and
[first-run guide](CODEX-FIRST-RUN.md). This is not a real user-profile install,
host-authenticated outcome, full isolation, or broad host lifecycle coverage.
[PR #57](https://github.com/luomo66ccff/reflexmesh/pull/57) merged the tested
tree `e40368503115ca9c7608f22f8a4c270f6926a8a1` unchanged; all five
configured jobs passed on the PR head and on
[main](https://github.com/luomo66ccff/reflexmesh/actions/runs/36088648306).

## Earlier merged milestone: Codex first connection
...
actually drained callbacks and from missing host results. The default remains
wait-for-drain; arbitrary caller-owned boundaries cannot opt into this fence.
...
## Reviewed increment: all-record ledger compaction
...
PR #20 exact head `c4471de112666bcee6c9ac883b70014b0b08e695` passed
[CI 35662335169](https://github.com/luomo66ccff/reflexmesh/actions/runs/35662335169):
Ubuntu/Node 22 **777/777**, Windows/Node 22/24 **776 passed, one existing skip**
...
- Explicit backup-bound preview/apply removes selected historical audit rows
  ...
  deletion; the global highwater row, six non-audit tables and existing metadata
  remain. Ordinary fresh ledgers remain 3, existing 4 is never downgraded.
- Read-only history and explicit one-batch archive query verify the full selected
  run before bounded output. Evidence views distinguish online from archived
  history. Missing/wrong archive is unverified, not missing-action evidence.
- One exclusive maintenance transaction requires all other connections closed,
  including idle readers. One-shot attempts retain unknown post-dispatch
  outcomes. Raw byte/BigInt digests, sequence exhaustion and metadata validation
  preserve the no-retry/idempotency boundary; no other evidence body is pruned.
- Legacy v1/seven-table backup/compaction artifacts remain unchanged. Schema 4
  uses v2/nine-table artifacts; a new full backup contains online references,
  not older external bodies. Archive availability remains separately unverified.
- Windows Node 22.23.2 and 24.19.0 passed **810 full-suite tests: 808 passed,
  zero failed, two existing local symlink skips** each. After a final test-only
  child-timeout fix/addition, all **34 focused tests** passed on both runtimes;
  the next full CI has 811 cases. All **nine offline demos** and the affected
  Node 22.16.0 **14 negative groups** passed. Old v1 artifacts and new v2/schema4
  readonly verification were exercised, including the affected old runtime.
- The retained synthetic lesson archives 42 then 1 audit rows, retrieves original
  run detail bytes, and verifies nine-table compaction plus replay/UNKNOWN/pair
  guards. Scoped independent review/root verification closed bounded-scan,
  encoding and test-child-cleanup defects; no remaining reviewed P1/P2.
  No real ledger, paid model, production restore or global runtime changes.
  See [guide](AUDIT-ARCHIVAL.md) and [validation t001](VALIDATION-AUDIT-ARCHIVAL-T001.md).
