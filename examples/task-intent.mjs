...
function startHere(directory, selectedKey) {
  ...
  const sourcePack = join(directory, 'source-pack.json');
  ...
  const commands = [command('list'), command('attention'),
    ...
    command('impact', ` --key ${shellQuote(selectedKey)} --candidate-pack ${shellQuote(candidate)}`),
